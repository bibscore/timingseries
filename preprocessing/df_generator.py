...
os.chdir(directory)
filename = "archive/southeast.csv"
csv_path = os.path.join(directory, filename)

ddf = dd.read_csv(filename, header=0, sep=',', parse_dates=['Data'])
df_southeast = ddf.compute()

df_southeast = df_southeast[(ddf['Data'] >= '2018-01-01') & (df_southeast['Data'] <= '2021-02-02')]

print(df_southeast)

df_southeast.drop(['index'], inplace=True, axis=1)
df_southeast.columns = abbreviation
df_southeast['date_time'] = pd.to_datetime(df_southeast['date'] + ' ' +  df_southeast['hour'])
df_southeast.drop(['date','hour','regi','prov','wsnm','lat','lon','elvt'], inplace=True, axis=1)
df_southeast = clean_na(df_southeast)
df_southeast.reset_index(drop=True, inplace=True) 

output_filename = "cleaned_southeast"
df_southeast.to_parquet(output_filename, engine='pyarrow', compression='snappy', index=False, use_deprecated_int96_timestamps=True)
